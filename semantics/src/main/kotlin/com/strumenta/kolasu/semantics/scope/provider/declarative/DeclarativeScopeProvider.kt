...
open class DeclarativeScopeProvider(
    vararg rules: DeclarativeScopeProviderRule<out NodeLike>,
) : ScopeProvider {
    private val rules: MutableList<DeclarativeScopeProviderRule<out NodeLike>> = rules.sorted().toMutableList()

    fun addRule(rule: DeclarativeScopeProviderRule<out NodeLike>) {
        rules.add(rule)
        rules.sort()
    }
 ...
 **/
class DeclarativeScopeProviderRule<NodeTy : NodeLike>(
...
) : (ScopeProvider, NodeLike) -> ScopeDescription,
    Comparable<DeclarativeScopeProviderRule<out NodeLike>> {
    override fun invoke(
 ...
 **/
data class DeclarativeScopeProviderRuleContext<NodeTy : NodeLike>(
    val node: NodeTy,
