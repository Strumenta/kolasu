...
fun IrBuilderWithScope.companionGetter(irClass: IrClass) : IrExpression {
    val companionClass =
        irClass
            .companionObject() ?: throw IllegalStateException("Cannot find companion object for ${irClass.kotlinFqName}")
...
fun IrBuilderWithScope.conceptGetter(nodeSubclass: IrClass) : IrExpression {
    val companionClass =
        nodeSubclass
            .companionObject() ?: throw IllegalStateException("Cannot find companion object for ${nodeSubclass.kotlinFqName}")
    val conceptField = companionClass.properties.find { it.name.identifier == "concept" }!!
    val conceptInstance: IrExpression = irCall(conceptField.getter!!).apply {
        dispatchReceiver = companionGetter(nodeSubclass)
    }
    return conceptInstance
}

fun IrBuilderWithScope.attributeByName(pluginContext: IrPluginContext, nodeSubclass: IrClass, attributeName: String) : IrExpression {
