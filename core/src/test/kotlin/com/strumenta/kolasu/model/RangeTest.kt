...
data class MySetStatement(val specifiedRange: Range? = null) : ASTNode(specifiedRange)
...
class RangeTest {
    ...
    @test
    fun containsNode() {
        val before = ASTNode(Range(Point(1, 0), Point(1, 10)))
        val inside = ASTNode(Range(Point(2, 3), Point(2, 8)))
        val after = ASTNode(Range(Point(3, 0), Point(3, 10)))
        val range = Range(Point(2, 0), Point(2, 10))
