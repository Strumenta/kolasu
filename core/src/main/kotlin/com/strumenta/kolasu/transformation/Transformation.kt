...
open class ASTTransformer(
     ...
     */
    @JvmOverloads
    open fun transform(source: Any?, parent: ASTNode? = null): ASTNode? {
        if (source == null) {
            return null
        }
    ...
    private fun setChildren(
        ...
        node::class.processProperties { pd ->
            ...
            var childNodeTransformer = transformer.getChildNodeTransformer<ASTNode, Any, Any>(node::class, pd.name)
    ...
    protected open fun getSource(node: ASTNode, source: Any): Any {
        return source
    }
    ...
    inline fun <reified S : Any, T : ASTNode> registerNodeTransformer(
    ...
    fun <S : Any, T : ASTNode> registerNodeTransformer(kclass: KClass<S>, transformer: (S) -> T?):
        NodeTransformer<S, T> =
        registerNodeTransformer(kclass) { input, _, _ -> transformer(input) }

    inline fun <reified S : Any, reified T : ASTNode> registerNodeTransformer(): NodeTransformer<S, T> {
