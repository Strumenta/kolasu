            ...
            return when {
                ...
                pdValue is ReferenceByName<*> -> {
                    ...
                    refEO.eSet(
                        ...
                        (pdValue.referred as? ASTNode)?.getOrCreateEObject(ePackage, kolasuToEMFMapping)
                    )
                    refEO
                }
                pdValue is Result<*> -> {
                    ...
                    if (pdValue.root is ASTNode) {
                        resEO.eSet(
                            resEC.getEStructuralFeature("root"),
                            (pdValue.root as ASTNode)
...
fun ASTNode.toEObject(ePackage: EPackage, mapping: KolasuToEMFMapping = KolasuToEMFMapping()): EObject =
 ...
 */
fun ASTNode.getOrCreateEObject(ePackage: EPackage, mapping: KolasuToEMFMapping = KolasuToEMFMapping()): EObject =
...
class KolasuToEMFMapping {
    ...
    fun associate(node: ASTNode, eo: EObject) {
        nodeToEObjects[node] = eo
    }

    fun getAssociatedEObject(node: ASTNode): EObject? {
     ...
     */
    fun getOrCreate(node: ASTNode, eResource: Resource): EObject {
 ...
 * This method retrieves the EObject already built for this Node or create it if it does not exist.
 */
fun ASTNode.getOrCreateEObject(eResource: Resource, mapping: KolasuToEMFMapping = KolasuToEMFMapping()): EObject {
 ...
 */
fun ASTNode.toEObject(eResource: Resource, mapping: KolasuToEMFMapping = KolasuToEMFMapping()): EObject {
    try {
            ...
            if (pd.provideNodes) {
                if (pd.multiple) {
                    ...
                    (pd.value as List<*>?)?.forEach {
                        try {
                            val childEO = (it as ASTNode?)?.getOrCreateEObject(eResource, mapping)
                            elist.add(childEO)
                        } catch (e: Exception) {
                    ...
                    if (pd.value == null) {
                        ...
                        eo.eSet(esf, (pd.value as ASTNode).getOrCreateEObject(eResource, mapping))
                    }
                }
            } else {
                if (pd.multiple) {
